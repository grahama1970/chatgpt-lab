...
6. Keep local package receipts under `artifacts/package/`; they prove local packaging only.
7. Change `control_plane.status` in `sources/source-manifest.json` from `BOOTSTRAP_PACKAGE` to `ACTIVE`.
8. Update `sources/control-plane/CURRENT_STATE.md` with the pushed commit and workflow run.
9. Paste or retain `PROJECT_INSTRUCTIONS.md` in the ChatGPT-Lab Project instructions.
