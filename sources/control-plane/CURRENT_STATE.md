...
| OpenCode GitHub event loop | `SOURCE_ADDED_UNPROVEN` | `.github/workflows/opencode-phatgpt.yml` can start OpenCode from issue/PR events or manual dispatch. It defaults to OpenCode OAuth model `gpt-5.5` with `medium` reasoning and allows dispatch overrides such as `deepseek-v4-flash`. Live run `28257495934` proved the action rejects provider-qualified `opencode/gpt-5.5`. `.opencode/agents/phatgpt-dispatcher.md` is the primary agent and routes to coder/reviewer/researcher subagents. A live successful event run is still required. |
