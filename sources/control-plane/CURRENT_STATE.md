...
| OpenCode GitHub event loop | `SOURCE_ADDED_UNPROVEN` | `.github/workflows/opencode-phatgpt.yml` can start OpenCode from issue/PR events or manual dispatch. `.opencode/agents/phatgpt-dispatcher.md` is the primary agent and routes to coder/reviewer/researcher subagents. A live event run is still required. |
