...
The preferred MVP trigger is event driven. `.github/workflows/opencode-phatgpt.yml` starts OpenCode on issue comments, PR comments, PR events, or manual dispatch. The default GitHub Action model is OpenCode OAuth `gpt-5.5` with `medium` reasoning; manual dispatch can override the model for smoke tests, for example `deepseek-v4-flash`. Local `opencode models` may display provider-qualified ids such as `opencode/gpt-5.5`, but the GitHub action's OpenCode provider expects the unqualified model id. The OpenCode primary agent is `.opencode/agents/phatgpt-dispatcher.md`; it routes to `@phatgpt-coder`, `@phatgpt-reviewer`, or `@phatgpt-researcher`. `opencode serve` is the local/Tailscale control surface when the loop needs to be driven from outside GitHub Actions. Cron is only a fallback watchdog.
