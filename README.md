...
The next dry-run route slice reads only local GitHub issue-thread fixtures under `examples/agent-harness/fixtures/`. `scripts/agent_ticket_route.py route-fixture` validates the active goal hash, trusted-human authority, explicit `next.subagent`, and the initial role allowlist, then emits `chatgpt_lab.route_decision.v1` with `would_mutate: false`. Legacy `phatgpt-task:v1` blocks are detected but not routed by the generic goal harness until a later adapter slice.

The shared JSON contract is intentionally close to GitHub's ticket/comment/label model. `schemas/agent-common.schema.json` defines common route, executor, and GitHub projection shapes; actionable `agent_handoff`, `human_interjection`, and `generated_ticket` blocks include a `github` projection that states the intended create/comment/label changes. Current code validates this projection but does not apply it.
