...
| REQ-WEB-010 | The MVP event loop must separate coder, reviewer, optional researcher, and dry-run deployer roles. | `.opencode/agents/phatgpt-dispatcher.md` routes to `phatgpt-coder`, `phatgpt-reviewer`, `phatgpt-researcher`, or `phatgpt-deployer`; each invocation handles at most one PR/issue and writes a receipt/comment trace. |
