...
- Researcher completion must be evidence-backed. Issue #18 showed a false-positive researcher loop: the worker validated the task block, repeatedly added `phatgpt-local-agent`, and commented `COMPLETED` without returning the requested local capability inventory. Researcher tasks that request evidence collection must now produce the requested evidence artifact or remain in `phatgpt-needs-task`; task-contract validation alone is not coder readiness. The fallback primitive is `scripts/phatgpt_capability_inventory.py`, which uses memory-first recall plus read-only local path validation and writes `artifacts/local-worker/issue-18/capability-inventory.json`. Completed researcher work uses `phatgpt-research-complete` and removes `phatgpt-needs-task` to avoid repeated watchdog pickup.
...
| 2026-06-27 | Researcher receipts must include requested evidence before coder routing | Issue #18 proved that `task_contract_validated` is insufficient: local-context inventory work must produce an inventory artifact/comment or fail closed without adding `phatgpt-local-agent`. |
